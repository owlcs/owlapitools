...
public class ComparisonReasoner implements OWLReasoner {
	...
	public ComparisonReasoner(OWLOntology o, OWLReasonerConfiguration config, OWLReasonerFactory... delegateFactories) {
		root = o;
		//		try {
		//		PrintStream out=new PrintStream("testOntology."+bean.getCurrentThreadCpuTime()+".owl");
		...
		for (int i = 0; i < delegateFactories.length; i++) {
			...
			OWLReasoner r = config == null ? delegateFactories[i].createReasoner(o) : delegateFactories[i].createReasoner(o, config);//, new SimpleConfiguration(new ConsoleProgressMonitor()));
	...
	public ComparisonReasoner(boolean tolerant,OWLOntology o, OWLReasonerConfiguration config, OWLReasonerFactory... delegateFactories) {
		this(o, config, delegateFactories);
		this.tolerateDifferences=tolerant;
	}
	...
	public String toString() {
		...
		b.append('\n');
		for (String s : methodNames) {
			int calls = timings.entrySet().iterator().next().getValue().get(s).values.size();
			b.append(s);
			b.append(',');
			b.append(calls);
			b.append(',');
	...
	private <T> T verify(List<T> objects) {
		for (int i = 1; i < objects.size(); i++) {
			if (!delegateEquals(objects.get(0), objects.get(i))) {
				...
				System.out.println(String.format(template, 0, delegates.get(0).getReasonerName(), objects.get(0).toString()));
				System.out.println(String.format(template, i, delegates.get(i).getReasonerName(), objects.get(i).toString()));
	...
	// never mind the warnings, this needs to be generic enough to accept anything
	...
	public void interrupt() {
		if (log) {
			System.out.println("r. interrupt(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		for (int i = 0; i < delegates.size(); i++) {
			try {
				...
				this.timings.get(delegates.get(i)).get("interrupt").values.add(elapsed);
	...
	public void flush() {
		...
		for (int i = 0; i < delegates.size(); i++) {
			try {
				...
				long elapsed = bean.getCurrentThreadCpuTime() - start;
				this.timings.get(delegates.get(i)).get("flush").values.add(elapsed);
			} catch (RuntimeException e) {
	...
	public List<OWLOntologyChange> getPendingChanges() {
		if (log) {
			System.out.println("r. getPendingChanges(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getPendingChanges").values.add(elapsed);
			objects.add(value);
		}
		try {
	...
	public Set<OWLAxiom> getPendingAxiomAdditions() {
		if (log) {
			System.out.println("r. getPendingAxiomAdditions(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getPendingAxiomAdditions").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Set<OWLAxiom> getPendingAxiomRemovals() {
		if (log) {
			System.out.println("r. getPendingAxiomRemovals(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		List<Set<OWLAxiom>> objects = new ArrayList<Set<OWLAxiom>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getPendingAxiomRemovals").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public void precomputeInferences(InferenceType... arg0) {
		if (log) {
			System.out.println("r. precomputeInferences(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		for (int i = 0; i < delegates.size(); i++) {
			try {
				long start = bean.getCurrentThreadCpuTime();
				...
				long elapsed = bean.getCurrentThreadCpuTime() - start;
				this.timings.get(delegates.get(i)).get("precomputeInferences").values.add(elapsed);
			} catch (RuntimeException e) {
	...
	public boolean isPrecomputed(InferenceType arg0) {
		if (log) {
			System.out.println("r. isPrecomputed(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("isPrecomputed").values.add(elapsed);
			objects.add(value);
		}
	//	try {
			return objects.get(0);
...
//		}
	...
	public Set<InferenceType> getPrecomputableInferenceTypes() {
		if (log) {
			System.out.println("r. getPrecomputableInferenceTypes(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getPrecomputableInferenceTypes").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public boolean isConsistent() {
		if (log) {
			System.out.println("r. isConsistent(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		List<Boolean> objects = new ArrayList<Boolean>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("isConsistent").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public boolean isSatisfiable(OWLClassExpression arg0) {
		if (log) {
			System.out.println("r. isSatisfiable(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		List<Boolean> objects = new ArrayList<Boolean>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("isSatisfiable").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLClass> getUnsatisfiableClasses() {
		if (log) {
			System.out.println("r. getUnsatisfiableClasses(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getUnsatisfiableClasses").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public boolean isEntailed(OWLAxiom arg0) {
		if (log) {
			System.out.println("r. isEntailed(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		List<Boolean> objects = new ArrayList<Boolean>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("isEntailed").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public boolean isEntailmentCheckingSupported(AxiomType<?> arg0) {
		if (log) {
			System.out.println("r. isEntailmentCheckingSupported(" + Arrays.toString(new Object[] { arg0 }) + ")");
		}
		List<Boolean> objects = new ArrayList<Boolean>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("isEntailmentCheckingSupported").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLClass> getTopClassNode() {
		if (log) {
			System.out.println("r. getTopClassNode(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		List<Node<OWLClass>> objects = new ArrayList<Node<OWLClass>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getTopClassNode").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLClass> getBottomClassNode() {
		if (log) {
			System.out.println("r. getBottomClassNode(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		List<Node<OWLClass>> objects = new ArrayList<Node<OWLClass>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getBottomClassNode").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLClass> getSubClasses(OWLClassExpression arg0, boolean arg1) {
		if (log) {
			System.out.println("r. getSubClasses(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getSubClasses").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLClass> getSuperClasses(OWLClassExpression arg0, boolean arg1) {
		if (log) {
			System.out.println("r. getSuperClasses(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		List<NodeSet<OWLClass>> objects = new ArrayList<NodeSet<OWLClass>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getSuperClasses").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLClass> getEquivalentClasses(OWLClassExpression arg0) {
		if (log) {
			System.out.println("r. getEquivalentClasses(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		List<Node<OWLClass>> objects = new ArrayList<Node<OWLClass>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getEquivalentClasses").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLClass> getDisjointClasses(OWLClassExpression arg0) {
		if (log) {
			System.out.println("r. getDisjointClasses(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		List<NodeSet<OWLClass>> objects = new ArrayList<NodeSet<OWLClass>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getDisjointClasses").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLObjectPropertyExpression> getTopObjectPropertyNode() {
		if (log) {
			System.out.println("r. getTopObjectPropertyNode(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		List<Node<OWLObjectPropertyExpression>> objects = new ArrayList<Node<OWLObjectPropertyExpression>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			Node<OWLObjectPropertyExpression> value = delegates.get(i).getTopObjectPropertyNode();
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getTopObjectPropertyNode").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLObjectPropertyExpression> getBottomObjectPropertyNode() {
		if (log) {
			System.out.println("r. getBottomObjectPropertyNode(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		List<Node<OWLObjectPropertyExpression>> objects = new ArrayList<Node<OWLObjectPropertyExpression>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			Node<OWLObjectPropertyExpression> value = delegates.get(i).getBottomObjectPropertyNode();
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getBottomObjectPropertyNode").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLObjectPropertyExpression> getSubObjectProperties(OWLObjectPropertyExpression arg0, boolean arg1) {
		if (log) {
			System.out.println("r. getSubObjectProperties(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		List<NodeSet<OWLObjectPropertyExpression>> objects = new ArrayList<NodeSet<OWLObjectPropertyExpression>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			NodeSet<OWLObjectPropertyExpression> value = delegates.get(i).getSubObjectProperties(arg0, arg1);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getSubObjectProperties").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLObjectPropertyExpression> getSuperObjectProperties(OWLObjectPropertyExpression arg0, boolean arg1) {
		if (log) {
			System.out.println("r. getSuperObjectProperties(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		List<NodeSet<OWLObjectPropertyExpression>> objects = new ArrayList<NodeSet<OWLObjectPropertyExpression>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			NodeSet<OWLObjectPropertyExpression> value = delegates.get(i).getSuperObjectProperties(arg0, arg1);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getSuperObjectProperties").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLObjectPropertyExpression> getEquivalentObjectProperties(OWLObjectPropertyExpression arg0) {
		if (log) {
			System.out.println("r. getEquivalentObjectProperties(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		List<Node<OWLObjectPropertyExpression>> objects = new ArrayList<Node<OWLObjectPropertyExpression>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			Node<OWLObjectPropertyExpression> value = delegates.get(i).getEquivalentObjectProperties(arg0);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getEquivalentObjectProperties").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLObjectPropertyExpression> getDisjointObjectProperties(OWLObjectPropertyExpression arg0) {
		if (log) {
			System.out.println("r. getDisjointObjectProperties(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		List<NodeSet<OWLObjectPropertyExpression>> objects = new ArrayList<NodeSet<OWLObjectPropertyExpression>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			NodeSet<OWLObjectPropertyExpression> value = delegates.get(i).getDisjointObjectProperties(arg0);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getDisjointObjectProperties").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLObjectPropertyExpression> getInverseObjectProperties(OWLObjectPropertyExpression arg0) {
		if (log) {
			System.out.println("r. getInverseObjectProperties(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		List<Node<OWLObjectPropertyExpression>> objects = new ArrayList<Node<OWLObjectPropertyExpression>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			Node<OWLObjectPropertyExpression> value = delegates.get(i).getInverseObjectProperties(arg0);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getInverseObjectProperties").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLClass> getObjectPropertyDomains(OWLObjectPropertyExpression arg0, boolean arg1) {
		if (log) {
			System.out.println("r. getObjectPropertyDomains(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		List<NodeSet<OWLClass>> objects = new ArrayList<NodeSet<OWLClass>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			NodeSet<OWLClass> value = delegates.get(i).getObjectPropertyDomains(arg0, arg1);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getObjectPropertyDomains").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLClass> getObjectPropertyRanges(OWLObjectPropertyExpression arg0, boolean arg1) {
		if (log) {
			System.out.println("r. getObjectPropertyRanges(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		List<NodeSet<OWLClass>> objects = new ArrayList<NodeSet<OWLClass>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			NodeSet<OWLClass> value = delegates.get(i).getObjectPropertyRanges(arg0, arg1);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getObjectPropertyRanges").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLDataProperty> getTopDataPropertyNode() {
		if (log) {
			System.out.println("r. getTopDataPropertyNode(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getTopDataPropertyNode").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLDataProperty> getBottomDataPropertyNode() {
		if (log) {
			System.out.println("r. getBottomDataPropertyNode(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		List<Node<OWLDataProperty>> objects = new ArrayList<Node<OWLDataProperty>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getBottomDataPropertyNode").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLDataProperty> getSubDataProperties(OWLDataProperty arg0, boolean arg1) {
		if (log) {
			System.out.println("r. getSubDataProperties(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		List<NodeSet<OWLDataProperty>> objects = new ArrayList<NodeSet<OWLDataProperty>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			NodeSet<OWLDataProperty> value = delegates.get(i).getSubDataProperties(arg0, arg1);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getSubDataProperties").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLDataProperty> getSuperDataProperties(OWLDataProperty arg0, boolean arg1) {
		if (log) {
			System.out.println("r. getSuperDataProperties(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		List<NodeSet<OWLDataProperty>> objects = new ArrayList<NodeSet<OWLDataProperty>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			NodeSet<OWLDataProperty> value = delegates.get(i).getSuperDataProperties(arg0, arg1);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getSuperDataProperties").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLDataProperty> getEquivalentDataProperties(OWLDataProperty arg0) {
		if (log) {
			System.out.println("r. getEquivalentDataProperties(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		List<Node<OWLDataProperty>> objects = new ArrayList<Node<OWLDataProperty>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			Node<OWLDataProperty> value = delegates.get(i).getEquivalentDataProperties(arg0);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getEquivalentDataProperties").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLDataProperty> getDisjointDataProperties(OWLDataPropertyExpression arg0) {
		if (log) {
			System.out.println("r. getDisjointDataProperties(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		List<NodeSet<OWLDataProperty>> objects = new ArrayList<NodeSet<OWLDataProperty>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			NodeSet<OWLDataProperty> value = delegates.get(i).getDisjointDataProperties(arg0);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getDisjointDataProperties").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLClass> getDataPropertyDomains(OWLDataProperty arg0, boolean arg1) {
		if (log) {
			System.out.println("r. getDataPropertyDomains(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		List<NodeSet<OWLClass>> objects = new ArrayList<NodeSet<OWLClass>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getDataPropertyDomains").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLClass> getTypes(OWLNamedIndividual arg0, boolean arg1) {
		if (log) {
			System.out.println("r. getTypes(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		List<NodeSet<OWLClass>> objects = new ArrayList<NodeSet<OWLClass>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getTypes").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLNamedIndividual> getInstances(OWLClassExpression arg0, boolean arg1) {
		if (log) {
			System.out.println("r. getInstances(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getInstances").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLNamedIndividual> getObjectPropertyValues(OWLNamedIndividual arg0, OWLObjectPropertyExpression arg1) {
		if (log) {
			System.out.println("r. getObjectPropertyValues(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		List<NodeSet<OWLNamedIndividual>> objects = new ArrayList<NodeSet<OWLNamedIndividual>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			NodeSet<OWLNamedIndividual> value = delegates.get(i).getObjectPropertyValues(arg0, arg1);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getObjectPropertyValues").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Set<OWLLiteral> getDataPropertyValues(OWLNamedIndividual arg0, OWLDataProperty arg1) {
		if (log) {
			System.out.println("r. getDataPropertyValues(" + Arrays.toString(new Object[] { arg0, arg1 }) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getDataPropertyValues").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public Node<OWLNamedIndividual> getSameIndividuals(OWLNamedIndividual arg0) {
		if (log) {
			System.out.println("r. getSameIndividuals(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getSameIndividuals").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public NodeSet<OWLNamedIndividual> getDifferentIndividuals(OWLNamedIndividual arg0) {
		if (log) {
			System.out.println("r. getDifferentIndividuals(" + Arrays.toString(Arrays.asList(arg0).toArray()) + ")");
		}
		List<NodeSet<OWLNamedIndividual>> objects = new ArrayList<NodeSet<OWLNamedIndividual>>();
		for (int i = 0; i < delegates.size(); i++) {
			long start = bean.getCurrentThreadCpuTime();
			NodeSet<OWLNamedIndividual> value = delegates.get(i).getDifferentIndividuals(arg0);
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getDifferentIndividuals").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public long getTimeOut() {
		if (log) {
			System.out.println("r. getTimeOut(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getTimeOut").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public FreshEntityPolicy getFreshEntityPolicy() {
		if (log) {
			System.out.println("r. getFreshEntityPolicy(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getFreshEntityPolicy").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public IndividualNodeSetPolicy getIndividualNodeSetPolicy() {
		if (log) {
			System.out.println("r. getIndividualNodeSetPolicy(" + Arrays.toString(Arrays.asList().toArray()) + ")");
		}
		...
		for (int i = 0; i < delegates.size(); i++) {
			...
			long elapsed = bean.getCurrentThreadCpuTime() - start;
			this.timings.get(delegates.get(i)).get("getIndividualNodeSetPolicy").values.add(elapsed);
			objects.add(value);
		}
		try {
			return verify(objects);
	...
	public void dispose() {
		...
		for (int i = 0; i < delegates.size(); i++) {
			try {
				...
				long elapsed = bean.getCurrentThreadCpuTime() - start;
				this.timings.get(delegates.get(i)).get("dispose").values.add(elapsed);
			} catch (RuntimeException e) {
