...
class FillerSuggestorImpl implements FillerSuggestor {
    ...
    private final AbstractOPMatcher possibleOPMatcher = new AbstractOPMatcher() {
        ...
        public boolean isMatch(OWLClassExpression c, OWLObjectPropertyExpression p,
            ...
            return !r.isSatisfiable(df.getOWLObjectIntersectionOf(c,
                    df.getOWLObjectAllValuesFrom(p, df.getOWLObjectComplementOf(f))));// SomeValuesFrom(p,
                                                                                      // f)));
        }
    };
    private final AbstractDPMatcher possibleDPMatcher = new AbstractDPMatcher() {
        ...
        public boolean isMatch(OWLClassExpression c, OWLDataPropertyExpression p,
                OWLDataRange f) {
            return !r.isSatisfiable(df.getOWLObjectIntersectionOf(c,
                    df.getOWLDataAllValuesFrom(p, df.getOWLDataComplementOf(f))));// SomeValuesFrom(p,
                                                                                  // f)));
        }
    };

    // private final AbstractOPMatcher possibleOPMatcher = new
    // AbstractOPMatcher() {
    // public boolean isMatch(OWLClassExpression c, OWLObjectPropertyExpression
    // p,
    // OWLClassExpression f) {
    // return r.isSatisfiable(df.getOWLObjectIntersectionOf(c,
    ...
    // }
    // };
    // private final AbstractDPMatcher possibleDPMatcher = new
    // AbstractDPMatcher() {
    // public boolean isMatch(OWLClassExpression c, OWLDataPropertyExpression p,
    // OWLDataRange f) {
    // return r.isSatisfiable(df.getOWLObjectIntersectionOf(c,
    ...
    public boolean isRedundant(OWLClassExpression c, OWLObjectPropertyExpression p,
        ...
        for (Node<OWLClass> node : r.getSubClasses(f, true)) { // check the
                                                               // direct
                                                               // subclasses
    ...
    //
    // public Set<OWLObject> getCurrentFillers(OWLClassExpression descr,
    // OWLPropertyExpression prop, boolean checkRedundancy) {
    ...
    // // now add complex fillers, by pulling the description apart, looking for
    // existentials/min cardis
    // // then verifying the fillers using isCurrentFiller()
    // FillerAccumulator<OWLClassExpression> acc = new
    // ExistentialObjectFillerAccumulator(objProp, r);
    // for (OWLClassExpression potentialFiller : acc.getFillers(descr)) {
    //
    // // filter out any descriptions that are equivalent to a named class
    ...
    // FillerAccumulator<OWLObject> acc = new DataFillerAccumulator(dataProp,
    // r);
    ...
    // getCurrentNamedFillersIncludingIndividuals(OWLClassExpression descr,
    ...
    //
    // NamedEntityFillerAccumulator accumulator = new
    // NamedEntityFillerAccumulator(prop, r);
    //
    ...
    // r.isSatisfiable(descr) &&
    // isSubclassOfSomeRestr(descr, prop, root)) {
    // fillers.addAll(getNamedFillersHelper(descr, prop, root,
    // checkRedundancy));
    // }
    //
    // return fillers;
    ...
    // * @throws OWLException
    // */
    // private Set<OWLClass> getNamedFillersHelper(OWLClassExpression descr,
    // OWLObjectPropertyExpression prop, OWLClassExpression root, boolean
    // checkRedundancy) {
    // Set<OWLClass> fillers = new HashSet<OWLClass>();
    //
    ...
    // if (r.isSatisfiable(subclass) && isSubclassOfSomeRestr(descr, prop,
    // subclass)) {
    // isCurrent = false;
    // fillers.addAll(getNamedFillersHelper(descr, prop, subclass,
    // checkRedundancy));
    // }
    // }
    ...
    // }
    //
    // public Set<OWLObject> getSanctionedFillers(OWLClassExpression descr,
    // OWLPropertyExpression prop, boolean recursive) {
    // Set<OWLObject> fillers = new HashSet<OWLObject>();
    //
    // if (prop instanceof OWLObjectPropertyExpression){
    // OWLObjectPropertyExpression objProp = (OWLObjectPropertyExpression) prop;
    // final OWLClassExpression globalAssertedRange =
    // getGlobalAssertedRange(objProp);
    // fillers.addAll(getSanctionedNamedFillers(descr, objProp, recursive,
    // globalAssertedRange));
    ...
    // }
    //
    // public Set<OWLClass> getSanctionedNamedFillers(OWLClassExpression descr,
    // OWLObjectPropertyExpression prop) {
    // return getSanctionedNamedFillers(descr, prop, true,
    // getGlobalAssertedRange(prop));
    // }
    //
    // private Set<OWLClass> getSanctionedNamedFillers(OWLClassExpression descr,
    // OWLObjectPropertyExpression prop, boolean recursive, OWLClassExpression
    // root) {
    // Set<OWLClass> fillers = new HashSet<OWLClass>();
    // for (OWLClass possibleFiller : getPossibleNamedFillers(descr, prop,
    // recursive, root)) {
    // if (!isRedundantFiller(possibleFiller, prop, descr)) { // as we've
    // already established possible
    // fillers.add(possibleFiller);
    // }
    // }
    // return fillers; // we don't filter as this constrains the superclasses
    // that are shown
    // }
    //
    // public Set<OWLClass> getPossibleNamedFillers(OWLClassExpression descr,
    // OWLObjectPropertyExpression prop) {
    // return getPossibleNamedFillers(descr, prop, true,
    // getGlobalAssertedRange(prop));
    // }
    //
    // public Set<OWLClass> getPossibleNamedFillers(OWLClassExpression descr,
    // OWLObjectPropertyExpression prop, boolean recursive) {
    // return getPossibleNamedFillers(descr, prop, recursive,
    // getGlobalAssertedRange(prop));
    // }
    //
    // public Set<OWLClass> getPossibleNamedFillers(OWLClassExpression descr,
    // OWLObjectPropertyExpression prop, boolean recursive, OWLClassExpression
    // root) {
    // Set<OWLClass> notFillers = getImpossibleNamedFillers(descr, prop,
    // recursive, root);
    ...
    // }
    //
    // public Set<OWLClass> getImpossibleNamedFillers(OWLClassExpression descr,
    // OWLObjectPropertyExpression prop) {
    // final OWLClassExpression globalRange = getGlobalAssertedRange(prop);
    // Set<OWLClass> impossibleNamedFillers = getImpossibleNamedFillers(descr,
    // prop, true, globalRange);
    //
    // // get everything outside the range
    // impossibleNamedFillers.addAll(ReasonerHelper.getDescendants(df.getOWLObjectComplementOf(globalRange),
    // r));
    ...
    // OWLObjectSomeValuesFrom owlSome =
    // df.getOWLObjectSomeValuesFrom(df.getOWLObjectInverseOf(prop), descr);
    // OWLObjectIntersectionOf expr = df.getOWLObjectIntersectionOf(root,
    // df.getOWLObjectComplementOf(owlSome));
    //
